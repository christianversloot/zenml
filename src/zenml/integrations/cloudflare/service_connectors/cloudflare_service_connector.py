...
The CloudFlare Service Connector allows you to authenticate to CloudFlare
services and access resources using the ZenML framework. The connector
currently provides support for R2 buckets.
...
""",
    ...
    auth_methods=[
        AuthenticationMethodModel(
            ...
            description="""
Long-lived AWS credentials consisting of an access key ID and secret access
key associated with a CloudFlare user (potentially bucket-specific).
""",
    ...
    resource_types=[
        ResourceTypeModel(
            ...
            resource_type=R2_RESOURCE_TYPE,
            description="""
Allows users to connect to R2 buckets. When used by Stack Components, they
are provided a pre-configured boto3 S3 client instance to the R2 bucket.
""",
            auth_methods=AWSAuthenticationMethods.values(),
            # Request an R2 bucket to be configured in the
            # connector or provided by the consumer
            supports_instances=True,
            logo_url="https://public-flavor-logos.s3.eu-central-1.amazonaws.com/artifact_store/cloudflare.png",
            emoji=":package:",
        ),
    ],
)
...
class AWSServiceConnector(ServiceConnector):
    ...
    def _get_iam_policy(
        ...
        """
        if resource_type == R2_RESOURCE_TYPE:
            if resource_id:
    ...
    def _configure_local_client(
        ...
        **kwargs: Any,
        ...
        if resource_type in [AWS_RESOURCE_TYPE, R2_RESOURCE_TYPE]:
            session, _ = self.get_boto3_session(
                self.auth_method,
                resource_type=resource_type,
                resource_id=self.resource_id,
